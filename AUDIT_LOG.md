...
| Date | Change |
|---|---|
| 2026-07-14 | **CLAW-047b — bot GitHub auth pivots PAT → GitHub App (CLI-mintable); fleet migrated to IMG7 then IMG8.** Mike wants to mint scoped tokens from the local CLI, which fine-grained PATs can't do (GitHub makes PAT creation UI-only — no gh/API). Switched to ONE shared "Oasis Claw Bots" App: each bot's image ships `git-credential-oasis-gh-app` (python + openssl, zero deps) that mints a SHORT-LIVED per-repo-scoped installation token ON DEMAND (JWT RS256 → installation-token API, down-scoped to the repo being pushed, cached ~55min, fail-open). Entrypoint prefers App mode (`GH_APP_ID` → helper `oasis-gh-app` + `credential.useHttpPath=true`), falls back to PAT (`GH_TOKEN` → `oasis-gh`), else anon. claw-git gained `app-init` (store App id+keypath in `~/.oasis-claw/gh-app.json`, key never in repo) + `app-set <bot> --repos` (inject App creds + scope, recreate, best-effort live-mint verify) + Makefile `git-app-init/git-app-set`; `list` shows app/pat mode. Plumbing UNIT-TESTED (JWT signature verifies via openssl; request/allowlist parsing) + container-validated in IMG8 (helper present, fail-open exit 0, graceful with throwaway key). LIVE pending Mike creating the App + key + installs. **Fleet migration:** all 7 bots recreated onto IMG7 (`4f7bc26e`, then IMG8 `cc9ea6f` carries the App helper) so git+gh+guardrail ship everywhere by default — VH via the 4-file sandbox stack, egress lock re-verified (example.com=000 blocked, anthropic reachable, sidecars reachable, github.com=000 → VH still can't reach github, egress decision still open). Runbook §2 = App setup (Mike's one-time UI steps); §2-ALT = PAT fallback; Workflows:write now in the default permission set (bots edit `.github/workflows/*`). Also: `bounty_hunter` created as private `oasis-main/bounty_hunter` (local venture-portfolio/bounty-hunter repointed); ButterBolt scope = `oasis-firmware` only; scoped `openclaw-tf-bot` IAM draft (bounded-PowerUser, assume-role) landed in oasis-cloud-admin (gated off, unapplied). |
