...
This skill drives the `plugins.entries.browser` plugin (openclaw `extensions/browser/`).
The plugin runs Chromium via `playwright-core`, exposes a local bridge server with
control-auth, manages profiles + downloads, and gates `evaluate()` (arbitrary JS
in pages) behind a default-off toggle. Auditor: deep-walk
`extensions/browser/index.ts`, `plugin-registration.ts`, and the `src/browser/`
sources covering `bridge-server.ts`, `control-auth.ts`, `request-policy.ts`,
`cdp-reachability-policy.ts`, `navigation-guard.ts`, `csrf.ts`,
`pw-tools-core.interactions.ts` (search for the `evaluate` path),
`browser-profiles.ts`, and the SSRF-related tests.
